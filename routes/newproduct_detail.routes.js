...
console.log(new_product_detail);
  ...
  try{
        await product_detailsModel.create({
            ...
            cat_id :  new_product_detail.cat_id,
            // sub_cat_id:req.body.sub_cat_id,
            breed_type : new_product_detail.breed_type,
            pet_type : new_product_detail.pet_type,
            age : new_product_detail.age,
            threshould : req.body.threshould || "",
            product_name : new_product_detail.product_name || "",
            product_discription : req.body.product_discription || "",
            product_img : new_product_detail.product_img || [],
            related :  "",
            count :  0,
            status :  "true",
...
router.get('/deletes', function (req, res) {
      newproduct_detailModel.remove({}, function (err, user) {
          if (err) return res.status(500).send("There was a problem deleting the user.");
             res.json({Status:"Success",Message:"New Product Details Deleted", Data : {} ,Code:200});     
      });
});


router.post('/edit', function (req, res) {
        newproduct_detailModel.findByIdAndUpdate(req.body._id, req.body, {new: true}, function (err, UpdatedDetails) {
            if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
