...
router.post('/create', async function(req, res) {
  ...
  try{
       ...
       if(phone !== null){
        ...
        var dumbell = "VOXITW";
        var tye = 0;
        var baseurls = "https://api.msg91.com/api/v5/otp?template_id=6163d627b7fce00aa76f1654&mobile=91"+req.body.user_phone+"&authkey=368262AUIWMydH6615ee50aP1";
...
//         //   "&type=" +
//         //   tye;
//         var baseurls = "https://api.msg91.com/api/v5/otp?template_id=6163d627b7fce00aa76f1654&mobile=91"+9003525711+"&authkey=368262AUIWMydH6615ee50aP1";
//         requestss(baseurls, { json: true }, async (err, response, body) => {
//           if (err) {
//             return 
//           }
...
router.post('/verify_otp', function (req, res) {
        res.json({Status:"Success",Message:"OTP verified success",Data : {} , Code:200});
        // var baseurls = "https://api.msg91.com/api/v5/otp/verify?otp="+req.body.otp+"&mobile=91"+req.body.user_phone+"&authkey=368262AUIWMydH6615ee50aP1";
        // requestss(baseurls, { json: true }, async (err, response, body) => {
        //   console.log(body);
        //   if (err) {
        //     return 
        ...
        //   else{
        //       if(body.type == 'error'){
        //         res.json({Status:"Failed",Message:body.message,Data : {} , Code:404}); 
        //       }
        //       else{
        //         console.log("verifcation",body.message);
        //         res.json({Status:"Success",Message:body.message,Data : {} , Code:200});
        //       }
        //       }
        //  });
});
...
router.post('/mobile/resendotp', function (req, res) {
        ...
        //   "&type=" +
        //   tye;
        var baseurls = "https://api.msg91.com/api/v5/otp/retry?mobile=91"+req.body.user_phone+"&authkey=368262AUIWMydH6615ee50aP1&retrytype=text";
        requestss(baseurls, { json: true }, async (err, response, body) => {
           if (err) {
            return 
          }
...
router.post('/edit', function (req, res) {
        ...
        req.body.user_phone = 99999999999;
...
router.post('/delete_service',async function (req, res) {
  var user_details  =  await userdetailsModel.find({user_type:req.body.user_type});
  console.log(user_details.length);
  for(let a  = 0; a < user_details.length ; a++) {
     userdetailsModel.findByIdAndRemove(user_details[a]._id, function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
          // res.json({Status:"Success",Message:"User Details Deleted successfully", Data : {} ,Code:200});
      });
    if(a == user_details.length - 1){
